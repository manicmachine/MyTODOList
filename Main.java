	...
	public static void addEventMenu() {
		...
		do {				
			...
			if (input.equals("y") || input.equals("yes")) {
				// Check if the event occurs at the same time as another
				for (Event event : events) {
				...
				// There are no events yet to compare if it's at the same time
				events.add(tempEvent);
				break;
			}
			
		} while(true); // debug this whole loop and see why it prompts "Enter event name:" When it should break
	}
